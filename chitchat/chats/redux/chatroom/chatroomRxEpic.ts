...
import { config, store } from "../../chitchatFactory";
...
export const createPrivateChatRoomEpic = action$ => {
    return action$.ofType(CREATE_PRIVATE_CHATROOM)
        .mergeMap(action => ajax({
            method: "POST",
            url: `${config.api.group}/private_chat/create`,
            body: action.payload,
            headers: {
                "Content-Type": "application/json",
                "x-access-token": store.getState().authReducer.token
...
export const getPersistendMessage = (currentRid) => ({ type: GET_PERSISTEND_MESSAGE, currentRid });
