...
export const getOrgGroup = (team_id) => ({ type: GET_ORG_GROUP, payload: team_id });
...
export const getOrgGroup_Epic = action$ => (action$.ofType(GET_ORG_GROUP)
    .mergeMap(action => ajax.getJSON(`${config().api.group}/org?team_id=${action.payload}`, { "x-access-token": Store.getState().authReducer.token }).map(response => getOrgGroupSuccess(response))
...
export const createOrgGroup_Epic = action$ => (action$.ofType(CREATE_ORG_GROUP).mergeMap(action => ajax({
    method: "POST",
...
export const uploadGroupImage_Epic = action$ => (action$.ofType(UPLOAD_GROUP_IMAGE)
    .mergeMap(action => {
