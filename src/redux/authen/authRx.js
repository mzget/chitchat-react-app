...
export const authUser = (user) => ({ type: AUTH_USER, payload: user }); // username => ({ type: FETCH_USER, payload: username });
...
export const authUserEpic = (action$) => action$.ofType(AUTH_USER)
...
export const logoutUser_Epic = (action$) => action$.ofType(LOG_OUT)
    ...
    if (result.success) {
        ...
        InternalStore.dataManager.messageDAL.clearData((err) => console.warn(err));
        StalkBridge.StalkBridgeActions.stalkLogout();
