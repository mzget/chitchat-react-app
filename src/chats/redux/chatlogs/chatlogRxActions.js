...
const chatlogsActions_1 = require("./chatlogsActions");
...
const waitForRemovedRoom = (data) => __awaiter(this, void 0, void 0, function* () {
    let id = setInterval(() => {
        let { state } = configureStore_1.default.getState().chatlogReducer;
...
exports.updateLastAccessRoom_Epic = action$ => action$.ofType(UPDATE_LAST_ACCESS_ROOM)
    ...
    let roomAccess = configureStore_1.default.getState().chatlogReducer.get("roomAccess");
...
const getLastAccessRoomRequest = () => ({ type: exports.GET_LAST_ACCESS_ROOM });
...
exports.getLastAccessRoom_Epic = action$ => action$.ofType(chatlogsActions_1.STALK_INIT_CHATSLOG)
    .mergeMap(action => {
    let { _id } = configureStore_1.default.getState().userReducer.user;
