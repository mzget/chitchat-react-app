...
export const callling_Epic = (data) => (dispatch) => {
    ...
    if (server) {
        ...
        const source = Observable.fromPromise(callingApi.calling(InternalStore.config.apiKey, calllingType, target_ids, { user_id, room_id }));
        const obs = source.subscribe((x) => dispatch(CallingSuccess(x)), (err) => dispatch(CallingFailure(err)), () => console.log("OnCalling completed"));
    }
    else {
...
export const hangupVideoCall_Epic = (action$) => action$.ofType(HANGUP_CALL)
    ...
    if (!!server) {
        ...
        return Observable.fromPromise(callingApi.calling(InternalStore.config.apiKey, CallingEvents.HangupCall, target_ids, { user_id }));
    }
    else {
