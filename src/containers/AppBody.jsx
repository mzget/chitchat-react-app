...
var getview = function (props) {
    ...
    if (match.params.filter == "user") {
        return <ProfileDetailEnhancer_1.ProfileDetailEnhanced user={userReducer.user} teamProfile={userReducer.teamProfile} alert={onError}/>;
    }
    else if (match.path.match("chatroom")) {
        if (match.path.match("chatroom/chat")) {
            return <Chat_1.ChatPage match={match} onError={onError} history={history}/>;
        }
        else if (match.params.edit == "edit") {
            return <GroupDetailEnhancer_1.GroupDetailEnhanced onError={onError} onFinished={function () { return console.log("Finished"); }}/>;
        }
        else if (match.params.edit == "add_member") {
            return <AddMembers_1.AddMembersEnhanced match={match}/>;
        }
    }
    else {
        return <Post_1.Post />;
...
exports.AppBody = function (props) { return (<div>   {getview(props)}
    </div>); };
