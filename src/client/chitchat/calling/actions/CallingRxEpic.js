...
export const videoCall_Epic = (data) => dispatch => {
    ...
    let source = Observable.fromPromise(callingApi.calling(chitchatFac.config.Stalk.apiKey, CallingEvents.VideoCall, target_ids, { user_id, room_id }));
    let obs = source.subscribe(x => dispatch(videoCallSuccess(x)), (err) => dispatch(videoCallFailure(err)), () => console.log("completed"));
};
...
export const hangupVideoCall_Epic = action$ => action$.ofType(HANGUP_CALL)
    .mergeMap(action => {
    let chitchatFac = ChitChatFactory.getInstance();
    let backendFactory = BackendFactory.getInstance();
    let callingApi = backendFactory.getServer().getCallingAPI();
    let { target_ids, user_id } = action.payload;
    return Observable.fromPromise(callingApi.calling(chitchatFac.config.Stalk.apiKey, CallingEvents.HangupCall, target_ids, { user_id }));
})
