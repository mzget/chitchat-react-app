...
export const callling_Epic = (data) => dispatch => {
    ...
    if (server) {
        ...
        let source = Observable.fromPromise(callingApi.calling(chitchatFac.config.Stalk.apiKey, calllingType, target_ids, { user_id, room_id }));
        let obs = source.subscribe(x => dispatch(CallingSuccess(x)), (err) => dispatch(CallingFailure(err)), () => console.log("OnCalling completed"));
    }
    else {
