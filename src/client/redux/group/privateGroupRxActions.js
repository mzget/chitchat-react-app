...
export const getPrivateGroup_Epic = action$ => (action$.ofType(GET_PRIVATE_GROUP)
    .mergeMap(action => ajax.getJSON(`${config().api.group}/private_group`, { "x-access-token": Store.getState().authReducer.token })
...
export const createPrivateGroup = (group) => ({ type: CREATE_PRIVATE_GROUP, payload: group });
