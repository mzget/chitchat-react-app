...
export const getOrgGroup = (teamId: string) => ({ type: GET_ORG_GROUP, payload: teamId });
...
export const getOrgGroupEpic = (action$) => (
    action$.ofType(GET_ORG_GROUP).mergeMap((action) =>
        ajax.getJSON(`${config().group}/org?team_id=${action.payload}`,
...
export const uploadGroupImageEpic = (action$) => (
