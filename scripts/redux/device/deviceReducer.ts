...
export const DeviceInitialState = Record({
  ...
  platform: '',
  version: null,
  appState: null, //active, background, inactive
  netState: null,
  deviceToken: null,
  os: null,
  state: null
...
export function deviceReducer(state = initialState, action) {
  if (!(state instanceof DeviceInitialState)) return initialState.merge(state);
  ...
  switch (action.type) {
    ...
    case DeviceActionsType.GET_DEVICETOKEN_SUCCESS: {
      ...
      else return state.set("state", DeviceActionsType.GET_DEVICETOKEN_SUCCESS);
    }
